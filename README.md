...
> ###7) Dealer
> - model
> - participant 를 상속받은 딜러, 게임종료 여부 판별 및 최종 수익 계산을 진행
> ####
> - 필드
> ####
> - 메서드  
>   : 플레이어가 잃은 만큼 +, 얻은 만큼 -하여 딜러의 최종 수익 계산  
    : 현재 가지고 있는 카드가 16 이하인지 아닌지 판별하기  
>   : 카드를 추가로 지급받을 수 있는 상태인지 조회하기   
    (카드 총합이 16을 초과했는지를 기준으로)   
> :  
